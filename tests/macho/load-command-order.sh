...
# Only arm64 output is ad-hoc signed by default.
if [ $ARCH = arm64 ]; then sig="LC_CODE_SIGNATURE "; else sig=; fi

$CC --ld-path=$mold -o $t/exe $t/a.o -Wl,-rpath,@loader_path -mmacosx-version-min=13.0
[ "$(seq $t/exe)" = "LC_SEGMENT_64 LC_DYLD_CHAINED_FIXUPS LC_DYLD_EXPORTS_TRIE LC_SYMTAB LC_DYSYMTAB LC_LOAD_DYLINKER LC_UUID LC_BUILD_VERSION LC_SOURCE_VERSION LC_MAIN LC_LOAD_DYLIB LC_RPATH LC_FUNCTION_STARTS LC_DATA_IN_CODE $sig" ]

$CC --ld-path=$mold -dynamiclib -o $t/libb.dylib $t/b.o -Wl,-rpath,@loader_path -mmacosx-version-min=13.0
[ "$(seq $t/libb.dylib)" = "LC_SEGMENT_64 LC_ID_DYLIB LC_DYLD_CHAINED_FIXUPS LC_DYLD_EXPORTS_TRIE LC_SYMTAB LC_DYSYMTAB LC_UUID LC_BUILD_VERSION LC_SOURCE_VERSION LC_LOAD_DYLIB LC_RPATH LC_FUNCTION_STARTS LC_DATA_IN_CODE $sig" ]
