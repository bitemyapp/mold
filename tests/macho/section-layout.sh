...
# x86-64 stubs are 2-byte aligned with chained fixups, byte-aligned
# with classic dyld info.
if [ $ARCH = x86_64 ]; then
  [ "$(align $t/exe __stubs)" = 2^1 ]
  $CXX --ld-path=$mold -o $t/exe5 $t/a.o $t/b.o $t/c.o -Wl,-no_fixup_chains
  [ "$(align $t/exe5 __stubs)" = 2^0 ]
fi
