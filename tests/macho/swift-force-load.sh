...
nm $t/libfoo.dylib | grep -q 'D __swift_FORCE_LOAD_\$_swiftFoo'
...
otool -L $t/exe | grep -q 'libfoo.dylib.*weak'
nm -m $t/exe | grep -q 'undefined) weak external __swift_FORCE_LOAD_\$_swiftFoo (from libfoo)'
