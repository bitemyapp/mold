...
## Documented exceptions, continued
...
- **Synthetic local-symbol names in a `-r` output** (restated): the
  `LC<n>`/`l<nnn>` names ld-prime gives the anonymous literal and
  Objective-C atoms, and which atoms it names, are matched in kind but
  not one-for-one; mold's own numbering is equally valid input to the
  final link.
